...
(function () {
	...
	MobroSDK.init().then(() => {
		...
		MobroSDK.addChannelListener('general_processor_temperature', (data) => {
			charts.cpuTemp.chart.data.datasets[0].data[0] = parseFloat(
				data.payload.value
			);
			charts.cpuTemp.chart.data.datasets[0].data[1] = parseFloat(
				data.payload.value - settings.max
			);
		...
		MobroSDK.addChannelListener('general_processor_usage', (data) => {
			charts.cpuLoad.chart.data.datasets[0].data.push(
		...
		MobroSDK.addChannelListener('general_graphics_temperature', (data) => {
			charts.gpuTemp.chart.data.datasets[0].data[0] = parseFloat(
				data.payload.value
			);
			charts.gpuTemp.chart.data.datasets[0].data[1] = parseFloat(
				data.payload.value - settings.max
			);
		...
		MobroSDK.addChannelListener('general_graphics_usage', (data) => {
			charts.gpuLoad.chart.data.datasets[0].data.push(
				parseInt(data.payload.value)
			);
		...
		MobroSDK.addChannelListener('general_memory_usage', (data) => {
			charts.ramUsage.chart.data.datasets[0].data[0] = parseFloat(
				data.payload.value
			);
			charts.ramUsage.chart.data.datasets[0].data[1] = parseFloat(
				100 - data.payload.value
			);
		...
		MobroSDK.addChannelListener('general_memory_used', (data) => {
			memoryData.innerHTML = convert(parseFloat(data.payload.value))
				.from(data.payload.unit)
				.to('GB')
				.toFixed(2);
		});

		MobroSDK.emit('monitor:hardware').then((data) => {
			document.getElementById('mobro-cpu-name').innerHTML =
				data.processor.cpus[0].name;
			document.getElementById('mobro-gpu-name').innerHTML =
				data.graphics.gpus[0].name;
			document.getElementById('mobro-ram-data--total').innerHTML = bytes(
				data.memory.totalcapacity
			);
		});

		MobroSDK.emit('monitor:sensor:data', 'general_processor_temperature').then(
			(data) => {
		...
		MobroSDK.addChannelListener('theme_fan_speed_cpu', (data) => {
			if (data.payload && data.payload.sensortype) {
				if (!charts.cpuFan) {
					charts.cpuFan = createClosedDoughnuts(
						document.getElementById('fan_cpu-chart-doughnut')
					);
					cpuFan.style.display = 'block';
				}
				charts.cpuFan.chart.data.datasets[0].data[0] = parseFloat(
					data.payload.value
				);
				charts.cpuFan.chart.data.datasets[0].data[1] = parseFloat(
					data.payload.avg - data.payload.value
				);
		...
		MobroSDK.addChannelListener('theme_fan_speed_gpu', (data) => {
			if (data.payload) {
				if (!charts.gpuFan) {
					charts.gpuFan = createClosedDoughnuts(
						document.getElementById('fan_gpu-chart-doughnut')
					);
					gpuFan.style.display = 'block';
				}
				charts.gpuFan.chart.data.datasets[0].data[0] = parseFloat(
					data.payload.value
				);
				charts.gpuFan.chart.data.datasets[0].data[1] = parseFloat(
					data.payload.avg - data.payload.value
				);
		...
		MobroSDK.addChannelListener('theme_vram', (data) => {
			if (data.payload) {
				...
				vramData.innerHTML = convert(data.payload.value)
					.from(data.payload.unit)
					.to('GB')
					.toFixed(2);
			} else {
		...
		MobroSDK.addChannelListener('theme_vram_percentage', (data) => {
			if (data.payload) {
				if (!charts.vramUsage) {
					charts.vramUsage = createDoughnuts(
						document.getElementById('vram-chart-doughnut')
					);
					vram.style.display = 'block';
				}
				charts.vramUsage.chart.data.datasets[0].data[0] = parseFloat(
					data.payload.value
				);
				charts.vramUsage.chart.data.datasets[0].data[1] = parseFloat(
					100 - data.payload.value
				);
...
function configureChartJS() {
	...
	Chart.pluginService.register({
		...
		afterDraw: function (chart) {
			...
			if (
				type == 'doughnut' &&
				chart.canvas.getAttribute('data-border') === 'margins'
			) {
				...
				ctx.arc(
					width / 2,
					height / 2 + height / 21.05,
					chart.chart.controller.outerRadius - lineWidth / 2,
					chart.chart.config.options.rotation,
					chart.chart.config.options.rotation +
						(chart.chart.config.options.circumference *
							chart.chart.config.options.breakpoints.orange) /
							100
				);
				ctx.stroke();
				...
				ctx.arc(
					...
					chart.chart.config.options.rotation +
						(chart.chart.config.options.circumference *
							chart.chart.config.options.breakpoints.orange) /
							100,
					chart.chart.config.options.circumference * 0.1
				);
				ctx.stroke();
				...
				ctx.arc(
					...
					chart.chart.config.options.rotation +
						(chart.chart.config.options.circumference *
							chart.chart.config.options.breakpoints.red) /
							100,
					chart.chart.config.options.rotation +
						chart.chart.config.options.circumference
				);
				...
				ctx.arc(
					width / 2,
					height / 2 + height / 21.05,
					chart.chart.controller.outerRadius - lineWidth / 2 - spaceWidth / 1.5,
					chart.chart.config.options.rotation,
					chart.chart.config.options.rotation +
						chart.chart.config.options.circumference
				);
